...
{
public:
    PointCloudCostmapNode() : Node("pointcloud_costmap_node")
    {
        // Declare parameters
        ...
        this->declare_parameter("base_frame", "unilidar_lidar");
        ...
        this->declare_parameter("publish_tf", false);  // Whether to publish static TF
        this->declare_parameter("lidar_frame", "unilidar_lidar");
        this->declare_parameter("lidar_roll", 0.0);   // degrees
        this->declare_parameter("lidar_pitch", 00.0);  // degrees  
        this->declare_parameter("lidar_yaw", 0.0);    // degrees
        this->declare_parameter("lidar_x", 0.0);       // meters
        this->declare_parameter("lidar_y", 0.0);       // meters
        this->declare_parameter("lidar_z", 0.5);       // meters
        ...
        double lidar_roll = this->get_parameter("lidar_roll").as_double();
        double lidar_pitch = this->get_parameter("lidar_pitch").as_double();
        double lidar_yaw = this->get_parameter("lidar_yaw").as_double();
        double lidar_x = this->get_parameter("lidar_x").as_double();
        double lidar_y = this->get_parameter("lidar_y").as_double();
        double lidar_z = this->get_parameter("lidar_z").as_double();
        
        // Initialize TF2
        tf_buffer_ = std::make_shared<tf2_ros::Buffer>(this->get_clock());
        tf_listener_ = std::make_shared<tf2_ros::TransformListener>(*tf_buffer_);
        
        // Setup static transform if requested
        if (publish_tf) {
            setupStaticTransform(lidar_frame, lidar_x, lidar_y, lidar_z, 
...
private:
                            ...
                            double roll_deg, double pitch_deg, double yaw_deg)
    {
        ...
        RCLCPP_INFO(this->get_logger(), 
                   "Translation: [%.2f, %.2f, %.2f], Rotation: [%.1f°, %.1f°, %.1f°]",
                           ...
                           sensor_msgs::msg::PointCloud2& output_cloud)
    {
        try {
            ...
            if (!tf_buffer_->canTransform(base_frame_, input_cloud->header.frame_id,
                                        input_cloud->header.stamp, rclcpp::Duration::from_seconds(0.1))) {
            ...
            tf2::doTransform(*input_cloud, output_cloud, 
                           tf_buffer_->lookupTransform(base_frame_, input_cloud->header.frame_id,
