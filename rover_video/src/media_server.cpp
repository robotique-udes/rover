...
int screenshotIPCam(std::string camera_url, std::string folder) {
...
int recordingIpCam(std::string camera_url, std::string folder)
{
    cv::VideoCapture cap(camera_url);
    ...
    // Get frame width and height   /* ChatGPT gave me this, gotta look into it more */
...
int main()
{
    ...
    const std::string screenshot_folder_path = std::string(dir) + "/src/screenshots"; // Path necessary for the screenshots folder /* Look for const in doc, i think they are illegal in Rovus */
    const std::string recording_folder_path = std::string(dir) + "/src/recordings";   // Path necessary for the recordings folder

    createFolder(screenshot_folder_path);
    createFolder(recording_folder_path);
   ...
   while(status != 'e') // Keeps printing the menu until the user wants to quit
   {
        ...
        {
            case 1:
                screenshotIPCam(camera_url, screenshot_folder_path);
                status = 0;
                break;

            case 2:
                recordingIpCam(camera_url, recording_folder_path);
                status = 0;
                break;
