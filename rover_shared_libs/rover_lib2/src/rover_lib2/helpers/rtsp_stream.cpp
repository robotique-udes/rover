...
namespace RoverLib2
{
    ...
    bool hasRTSPStream(const std::string& url_, size_t timeoutMs_)
    {
        ...
        if (connect(sock, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0 && errno != EINPROGRESS)
        {
            close(sock);
            return false;
        }
        ...
        FD_ZERO(&wfds);
        FD_SET(sock, &wfds);
        timeval tv{static_cast<long>(timeoutMs_ / 1000), static_cast<long>((timeoutMs_ % 1000) * 1000)};
