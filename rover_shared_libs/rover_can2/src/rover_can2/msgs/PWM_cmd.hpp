...
namespace RoverCan2::Msgs
{
    ...
    eLoadMsgCode PwmCmd::_loadMsg(const CanMsg& msg_)
    {
    ...
    std::optional<CanMsg> PwmCmd::_getCanMsg(const uint8_t msgContentId_) const
    {
    ...
    uint8_t PwmCmd::_getMsgContentCount(void) const
    {
        return TO_UNDERLYING(eMsgContentID::eLAST);
    }

    PwmCmd::sMsgData& PwmCmd::data(void)
    {
        return _data;
    }
    
    const PwmCmd::sMsgData& PwmCmd::getData(void) const
    {
        return static_cast<const PwmCmd::sMsgData&>(_data);
    }
}  // namespace RoverCan2::Msgs
