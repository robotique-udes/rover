...
RtspPlayerWidget::RtspPlayerWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::RtspPlayerWidget),
      workerThread(new QThread(this)), gstreamerWorker(new GStreamerWorker()),
      reconnectTimer(new QTimer(this)), frameTimeoutTimer(new QTimer(this)),
      pipeline(nullptr), receivingFrames(false), inReconnectionMode(false) {
  ui->setupUi(this);

  gstreamerWorker->moveToThread(workerThread);
  connect(workerThread, &QThread::finished, gstreamerWorker,
          &QObject::deleteLater);

  connect(this, &RtspPlayerWidget::requestStartStream, gstreamerWorker,
          &GStreamerWorker::startPipeline);
  connect(this, &RtspPlayerWidget::requestStopStream, gstreamerWorker,
          &GStreamerWorker::stopPipeline);

  connect(gstreamerWorker, &GStreamerWorker::pipelineStarted, this,
          &RtspPlayerWidget::onPipelineStarted);
  connect(gstreamerWorker, &GStreamerWorker::errorOccurred, this,
          &RtspPlayerWidget::onErrorOccurred);
  ...
  connect(ui->stopButton, &QPushButton::clicked, this,
          &RtspPlayerWidget::stopStream);

  connect(ui->stopButton, &QPushButton::clicked, this, [this]() {
    ui->startButton->setEnabled(true);
    LOG_DEBUG("RtspPlayer",
              "Stop button clicked - Start button enabled directly");
  });

  connect(ui->startButton, &QPushButton::clicked, this,
          [this]() { startStream(ui->rtspUrlInput->text()); });
...
void RtspPlayerWidget::updateStatusIndicator(const QString &color) {
  ui->statusIndicator->setStyleSheet(
      QString("QFrame { border-radius: 10px; background-color: %1; }")
          .arg(color));
}