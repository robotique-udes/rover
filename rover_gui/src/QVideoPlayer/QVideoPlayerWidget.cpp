...
void QVideoPlayerWidget::connectUISignals(void)
{
    if (_playPauseButton)
    {
        connect(_playPauseButton, &QPushButton::clicked, this, &QVideoPlayerWidget::handlePlayPauseButton);

        if (_toggleViewButton)
        {
            connect(_toggleViewButton, &QPushButton::clicked, this, &QVideoPlayerWidget::onToggleView);
        }
    ...
    void QVideoPlayerWidget::tryReconnect(void)
    {
        ...
        {
            UI_LOG_INFO_RTSP(
                QString("Automatic reconnection attempt %1 of %2").arg(_reconnectAttempts).arg(MAX_RECONNECT_ATTEMPTS),
                _widgetId);
