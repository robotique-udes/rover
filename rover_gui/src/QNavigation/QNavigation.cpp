    ...
    _node(guiNode_)
{
    ...
    connect(ui->setGoalButton,
            ...
            [this]()
            {
                ...
                if (ui->inputName->text().isEmpty() || ui->inputLatitude->text().isEmpty()
    ...
    connect(ui->calculatePathButton, &QPushButton::clicked, this, &QNavigation::onCalculatePathClicked);

    // Connect the waypoint selection
    connect(ui->waypointList, &QListWidget::itemClicked, this, &QNavigation::onWaypointSelected);
...
void QNavigation::addWaypointToList(const QString& name, double latitude, double longitude)
{
    ...
    // Add to UI list
    QString displayText = QString("%1 (%2, %3)").arg(name).arg(latitude, 0, 'f', 6).arg(longitude, 0, 'f', 6);
