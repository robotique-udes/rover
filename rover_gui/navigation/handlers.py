import threading
from rover_msgs.srv import PanoControl, ScreenshotControl
...
class ScreenshotClient:
    def __init__(self, node):
        self.node = node
        self.cb_group = ReentrantCallbackGroup()
        self.screenshot_control_client = self.node.create_client(ScreenshotControl, 'control_screenshot', callback_group=self.cb_group)
        self.executor = MultiThreadedExecutor()
        self.executor.add_node(self.node)
        self.spin_thread = threading.Thread(target=self.executor.spin, daemon=True)
        self.spin_thread.start()
    ...
    def send_request(self, request, callback):
        ...
        future = self.screenshot_control_client.call_async(request)
        future.add_done_callback(internal_callback)
