...
class Rover_sim : public rclcpp::Node
{
public:
    Rover_sim();
...
Rover_sim::Rover_sim() : Node("rover_sim")
{
    _sub_arm_cmd = this->create_subscription<rover_msgs::msg::ArmCmd>("/rover/arm/cmd/in/teleop",
                                                                      1,
                                                                      std::bind(&Rover_sim::simCallback, this, std::placeholders::_1));
...
int main(int argc, char *argv[])
{
    ...
    rclcpp::spin(std::make_shared<Rover_sim>());
    rclcpp::shutdown();
}