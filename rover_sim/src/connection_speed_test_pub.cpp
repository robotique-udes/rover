...
class ConnectionSpeedPublisher : public rclcpp::Node
{
  public:
    ...
    {
        _speed_connection = 1000000;
        _connection_quality = -50;
        _publisher = this->create_publisher<rover_msgs::msg::AntennaStatus>("/rover/antenna/status", 10);
  ...
  private:
    ...
    {
        auto msg = rover_msgs::msg::AntennaStatus();
        msg.txrate = _speed_connection;
        msg.rssi = _connection_quality;
        msg.connected = true;
        _publisher->publish(msg);
        _speed_connection += 100000;
        _connection_quality -= 2;
        if (_connection_quality <= -90)
        {
            _connection_quality = -50;
        }
        if (_speed_connection >= 10900000)
        {
            _speed_connection = 1000000;
        }
    }

    rclcpp::Publisher<rover_msgs::msg::AntennaStatus>::SharedPtr _publisher;
