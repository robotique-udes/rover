...
class Aruco(Node):
    def __init__(self):
        ...
        main_loop = threading.Thread(target=self.detect_aruco, daemon=True)
        main_loop.start()
    ...
    def convert_to_gray(self, frame):
        try:
            ...
            self.get_logger().warn('Erreur de conversion en grayscale')
            return None
        else:
            return grey_frame
    ...
    def add_frame(self):
        try:
            ...
            self.get_logger().error('Erreur de communication avec la caméra, attente de 1 seconde')
            time.sleep(1)
            return 0
        else:
            ret, frame = cap.read()
            if frame is None:
                self.get_logger().warn('Image not loaded, trying again...')
                return 0
            else:
                self.frame = frame
