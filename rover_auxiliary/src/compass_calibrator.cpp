...
CompassCalibrator::CompassCalibrator(): Node("compass_calibrator")
{
    ...
    _sub_compassRaw = this->create_subscription<rover_msgs::msg::Compass>(
        "/rover/auxiliary/compass/raw",
        1,
        std::bind(&CompassCalibrator::compassCallback, this, std::placeholders::_1));

    _srv_compassCalib = this->create_service<rover_msgs::srv::CompassCalibration>(
        "/rover/auxiliary/compass/calibrate",
        std::bind(&CompassCalibrator::CB_srv, this, std::placeholders::_1, std::placeholders::_2));
}
...
void CompassCalibrator::changeHeadingZero()
{
    ...
    while (_calibratedHeading >= 360.0f)
        _calibratedHeading -= 360.0f;
    while (_calibratedHeading < 0.0f)
        _calibratedHeading += 360.0f;
}