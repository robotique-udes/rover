...
class GoalManager : public rclcpp::Node
{
  ...
  private:
    const std::string _dumpFileName = ament_index_cpp::get_package_share_directory("rover_gui")
                                      + "/../../../../src/rover/rover_gui/saved_files/navigation/" + FILE_NAME_ROUTE_DUMP;
...
GoalManager::GoalManager(): Node("goal_manager")
{
    ...
    _srvNewGoal = this->create_service<rover_msgs::srv::NewGpsGoal>(
        "/rover/drive_train/auto/manage_goal",
        std::bind(&GoalManager::CB_srvNewGoal, this, std::placeholders::_1, std::placeholders::_2));
}
                                ...
                                std::shared_ptr<rover_msgs::srv::NewGpsGoal::Response> response_)
{
    ...
    if (response_->status != "")
    {
        ...
        RCLCPP_WARN(this->get_logger(),
                    ("Bad request of type:\"" + std::to_string(request_->type) + "\", with specified index:\""
                     + std::to_string(request_->index) + "\", and with waypoints:[\"" + waypointStr + "]\" -> "
                     + response_->status)
                        .c_str());
    }
...
void GoalManager::dumpCurrentRouteToFile(void)
{
    ...
    if (!file)
    {
        RCLCPP_ERROR(this->get_logger(),
                     "Failed to dump updated route to file... Make sure following folder structure exist: %s",
                     _dumpFileName.c_str());
    }
    else
    {
