...
void AntennaNode::retrieveDriverInfosAndPublish(void)
{
    ...
    switch (result)
    {
        ...
        case eAntennaCode::FAILURE_DEVICE_OFFLINE:
            RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), LOGGER_THROTTLE_MS, "Base antenna is offline");
            break;
        case eAntennaCode::FAILURE_SESSION_EXPIRED:
            RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), LOGGER_THROTTLE_MS, "Access forbidden, check antenna connection or your credentials in doc");
            break;
        case eAntennaCode::FAILURE_PARSING_ERROR:
            RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), LOGGER_THROTTLE_MS,
        ...
        case eAntennaCode::FAILURE_UNKNOWN:
            [[fallthrough]];
        default:
            RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), LOGGER_THROTTLE_MS, "Couln't retrieve data link info, unknown error");
            break;
    }
}