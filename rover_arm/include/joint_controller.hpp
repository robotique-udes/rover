...
class JointController : public RobotController
{
  public:
        ...
        std::array<float, TO_UNDERLYING(eJoyInput::eLAST)> inputArray_) override
    {
        ...
        {
            jointCommands[TO_UNDERLYING(eJointIndex::JL)] = -1.0F * getMaxVelocity(ARM_CONFIGURATION::JL::ID);
        }
        ...
        {
            jointCommands[TO_UNDERLYING(eJointIndex::GRIPPER_TILT)] = getMaxVelocity(ARM_CONFIGURATION::GRIPPER_TILT::ID);
        }
        else if (_joyManager.isPressed(KEYBINDINGS::JOINT::WRIST_DOWN))
        {
            jointCommands[TO_UNDERLYING(eJointIndex::GRIPPER_TILT)] = -1.0F * getMaxVelocity(ARM_CONFIGURATION::GRIPPER_TILT::ID);
        }
